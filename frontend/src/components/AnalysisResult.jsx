...
const AnalysisResult = ({ data }) => {
  ...
  // If LLM wraps in ```markdown ... ```, strip so inner content is parsed as markdown
  const trimCodeFence = (raw) => {
    if (!raw || typeof raw !== "string") return "";
    const s = raw.trim();
    if (s.startsWith("```")) {
      const afterBackticks = s.slice(3).replace(/^(?:markdown|md)\s*\n?/i, "");
      const end = afterBackticks.lastIndexOf("```");
      if (end !== -1) return afterBackticks.slice(0, end).trim();
      return afterBackticks.trim();
    }
    return s;
  };
  ...
  const fetchAI = async () => {
    try {
      ...
      const url = `http://localhost:4000/api/ai-explain?repoUrl=${encodeURIComponent(repoUrl)}`;
    ...
    <div className="analysis-page">
      <div className="analysis-container">
        ...
        <div className="explanation">
          <div className="markdown ai-explanation-markdown">
            {aiText ? (
              <div
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(marked.parse(trimCodeFence(aiText))),
                }}
              />
            ) : (
              <p>Click the button above to get an AI summary of the risk analysis.</p>
            )}
          </div>
        </div>
      </div>
